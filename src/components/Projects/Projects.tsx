...
const Projects = ({ reference, onSeeMore }: ProjectsProps) => {
  ...
  return (
    <section ref={reference} className="flex flex-col justify-center">
      <SectionWrapper
        containerClass={"container-1280"}
        paddingSectionClass={"py-[1em] sm:py-[2em] lg:py-[8em]"}
      >
        <ScrollAnimationWrapper>
          <div className="py-6">
            <div
              className="max-w-5xl px-6 mx-auto text-center flex items-center md:items-center flex-col"
              id="project"
            >
              <h2 className="text-2xl font-semibold w-fit flex">
                {" "}
                <ThreeDCubeSphere /> My Projects
              </h2>
            </div>
          </div>
        </ScrollAnimationWrapper>
...
const Cards = ({
  ...
  if (typeof description === "string") {
    ...
    descString = description.props?.children
    ...
    <>
      <div className="card lg:card-side bg-secondary text-slate-200 shadow container h-full">
        <figure className="w-full lg:w-1/2 ">
          <img
            src={img}
            className="h-fit sm:h-full md:h-full w-full object-cover"
            alt="Album"
          />
        </figure>
        <div className="card-body lg:w-1/2 w-full">
          ...
          <p className="text-slate-400 text-lg">
            ...
            {isLong && (
              <span
                className="text-info cursor-pointer ml-2 underline"
                onClick={onSeeMore}
              >
                See more
              </span>
